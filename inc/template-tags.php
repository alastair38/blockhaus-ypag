...
function blockhaus_display_social_profiles() {
	...
	if($facebook || $instagram || $linkedin || $tiktok || $twitter || $youtube):
		$profiles = '<ul class="wp-container-1 wp-block-social-links flex items-end justify-between has-normal-icon-size has-icon-color is-style-logos-only gap-3">';
...
function blockhaus_post_edit_link()  {
		...
		if(current_user_can( 'edit_post', $page_id ) && !is_search()):
